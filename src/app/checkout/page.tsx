...
import { Check, CreditCard, QrCode, ShieldCheck, Truck, Upload } from "lucide-react";
...
export default function CheckoutPage() {
  ...
  const sub = subtotal();
  const total = useMemo(() => sub + (items.length ? DELIVERY_FEE : 0), [sub, items.length]);
  ...
  const placeOrder = async (e: React.FormEvent) => {
    ...
    if (!name || !email || !phone || !address || !requiredBy) {
      toast.error("Fill in delivery details");
      return;
    }
    setSubmitting(true);
    try {
      ...
      const order = await createOrder({
        ...
        total,
        requiredBy,
        status: method === "khalti" || method === "esewa" ? "pending_payment" : method === "qr_upload" ? "received" : "received",
        payment: {
          method,
          status: method === "qr_upload" && screenshotUrl ? "pending_review" : method === "cod" ? "unpaid" : "unpaid",
          screenshotUrl,
          amount: total
        }
      });
      ...
      toast.success("Order placed", { description: `Your code is ${order.code}` });
    } catch (err) {
      console.error(err);
        ...
        <div className="space-y-8 lg:col-span-7">
          <fieldset className="glass rounded-3xl p-7">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <label className="md:col-span-2">
                <span className="field-label">Email <span className="text-muted-foreground">(used for tracking)</span></span>
                <input type="email" className="field" value={email} onChange={(e) => setEmail(e.target.value)} required />
              </label>
              <label className="md:col-span-2">
                <span className="field-label">Address</span>
                <textarea className="field" value={address} rows={2} onChange={(e) => setAddress(e.target.value)} required />
              </label>
              <label className="md:col-span-1">
              ...
              <label className="md:col-span-1">
                ...
                <input type="datetime-local" className="field" value={requiredBy} onChange={(e) => setRequiredBy(e.target.value)} required />
              </label>
              <label className="md:col-span-2">
                <span className="field-label">Notes</span>
                <textarea className="field" rows={2} value={note} onChange={(e) => setNote(e.target.value)} placeholder="Anything else we should know?" />
              </label>
            </div>
          </fieldset>
          ...
          <fieldset className="glass rounded-3xl p-7">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <PaymentOption
                active={method === "qr_upload"}
                onClick={() => setMethod("qr_upload")}
                icon={<QrCode className="h-5 w-5" />}
                title="QR / Bank · Upload screenshot"
                description="Scan our QR (eSewa / Khalti / mobile banking), pay, and upload proof. We'll verify in under an hour."
              />
              <PaymentOption
                active={method === "khalti"}
                onClick={() => setMethod("khalti")}
                icon={<span className="font-display text-base font-bold">K</span>}
                title="Khalti"
                description="Pay instantly with Khalti wallet (sandbox-ready)."
              />
              <PaymentOption
                active={method === "esewa"}
                onClick={() => setMethod("esewa")}
                icon={<span className="font-display text-base font-bold">e</span>}
                title="eSewa"
                description="Pay instantly with eSewa wallet (sandbox-ready)."
              />
              <PaymentOption
                active={method === "cod"}
                ...
                description="Pay our delivery rider when the cake arrives."
              />
            </div>
              ...
              <div className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-2">
                <div className="rounded-2xl border border-dashed border-border bg-surface/50 p-5 text-center">
                  <Image
                    ...
                    width={180}
                    height={180}
                    className="mx-auto rounded-2xl bg-background object-contain p-3"
                  />
                  ...
                  <p className="mt-2 text-xs text-muted-foreground">
                    Use any wallet. Admin can swap this QR from Admin → Content.
                  </p>
                </div>
                <label className="flex cursor-pointer flex-col gap-3 rounded-2xl border border-dashed border-border bg-surface/50 p-5 text-sm text-muted-foreground transition-colors hover:border-secondary/40 hover:text-foreground">
                  ...
                  />
                  {screenshot ? (
                    <span className="rounded-xl border border-border bg-surface p-3 text-foreground">{screenshot.name}</span>
                  ) : (
                    <span>JPG / PNG · we verify within an hour.</span>
                  )}
                </label>
              </div>
            )}
            ...
            {(method === "khalti" || method === "esewa") && (
              <div className="mt-6 rounded-2xl border border-border bg-surface/50 p-5 text-sm text-muted-foreground">
                <p className="mb-2 inline-flex items-center gap-2 text-foreground"><ShieldCheck className="h-4 w-4 text-secondary" /> Sandbox mode</p>
                <p>
                  Add <code className="rounded bg-surface-container px-1.5 py-0.5">NEXT_PUBLIC_{method.toUpperCase()}_*</code> env
                  vars in Vercel to enable live payments. We'll mark the order as <em>pending payment</em> and confirm once the
                  gateway webhook fires.
                </p>
              </div>
            )}
          </fieldset>
        ...
        <aside className="lg:col-span-5">
          <div className="glass sticky top-28 space-y-5 rounded-3xl p-7">
            ...
            <p className="text-center text-[11px] text-muted-foreground">
              By placing the order you agree to be contacted about its progress.
            </p>
          </div>
        </aside>
      </form>
...
function PaymentOption({
  ...
  return (
    ...
    >
      <span className="mt-0.5 inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-surface ring-1 ring-border">
        {icon}
      </span>
